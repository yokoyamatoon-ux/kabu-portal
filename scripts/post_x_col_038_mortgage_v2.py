...
def post_col_038_thread_v2():
    ...
    image_path = os.path.join("image", "column", "Column20260701.png")
    if not os.path.exists(image_path):
        print(f"❌ エラー: 画像ファイルが見つかりません: {image_path}")
        return

    # --- 原稿と文字数ポイント検証 ---
    parent_text = """フォッフォッフォ！「住宅ローン金利が引き下げ！よしマイホームを買うぞ！」と舞い上がっておる者はおらんかの？

「金利上昇が怖いから一生『賃貸』のまま暮らすのが最強！」というマネ太の疑問に喝じゃ！

実は金利上昇は、賃貸の家賃にも波及するのじゃよ。解説するぞい！👇"""

    reply_text = """同等スペックの家なら、生涯の総住居費は持ち家も賃貸もほぼ同じと言われておる。

じゃが、金利高騰は大家のアパートローンや建物の修繕コストを上げるため、中長期的には家賃に転嫁されるのじゃ。

持ち家 vs 賃貸の生涯コスト比較はこちら！👇
https://okane-no-manabi.jp/column/col_038/"""
    ...
    # 履歴への記録
    full_text = f"[V2-Re-post] {parent_text}\n\n[Thread Reply]\n{reply_text}"
    log_post_to_history(
    ...
    print("🎉 スレッド全体の再投稿および履歴記録が完了しました！")

if __name__ == "__main__":
    post_col_038_thread_v2()
