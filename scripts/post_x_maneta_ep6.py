...
def post_maneta_diary_ep6():
    ...
    # 画像パス
    img_dir = r"D:\Antigravity\Kabu\manga\maneta\06"
    img_files = ["maneta20260508_01.png", "maneta20260508_02.png"]
    img_paths = [os.path.join(img_dir, f) for f in img_files]

    # ポスト1: フック (マンガ画像付き)
    text1 = """【新着マンガ】マネ太の投資日記 第6話
マネ太が「ポートフォリオ」というカッコいい言葉を覚えたようじゃ！💼

しかし、中身の銘柄選びより大切な「大枠の設計図」を知っておるかの？
「卵を一つのカゴに盛るな」の格言と、アセットアロケーションの違いを解説するぞい！🥚✨

#投資初心者 #資産運用 #マンガで学ぶ"""

    # ポスト2: 誘導 (スレッド)
    text2 = """全体のお皿（資産配分）をどう分けるかで、運用の成績の9割が決まると言われておる。
初心者はまず「半分は現金」「半分は全世界株（オルカン）」のシンプルな配分から始めるのがおすすめじゃ！

心のクッションを持って長く続けるのが秘訣ぞい。👇
https://okane-no-manabi.jp/maneta_diary/6/"""
