...
def main():
    print("Starting browser-based X post for Money Secret 23 (SNS Pump and Dump)...")

    # スレッド原稿定義
    parent_text = (
        "フォッフォッフォ！SNSでよく見かける「この株は10倍になる！絶対に買い！」という最強株情報のウラ事情じゃ！\n\n"
        "表向きは親切なおすすめ情報のフリをして、裏ではフォロワーをカモにして売り抜ける「ポンプ＆ダンプ」の闇と、日米の逮捕事例を解説するぞい！👇"
    )
    image_path = r"D:\Antigravity\Kabu\image\manga\urakane\urakane20260713_01.png"

    reply_text = (
        "日本でも2024年にSNSの虚偽情報による「風説の流布」で初の課徴金勧告が出され、明確な処罰対象になったのじゃ。\n\n"
        "インフルエンサーの買い煽りに騙されず、一次情報をチェックする自衛力を身につけるのじゃ！👇\n"
        "https://okane-no-manabi.jp/money_secret/23/"
    )
    ...
    if res.get("success"):
        print("✅ ウラ金さんファイル23宣伝スレッド投稿成功！")
        # 履歴記録
        full_text = f"[urakane_23]\n{parent_text}\n\n[Reply]\n{reply_text}"
        log_post_to_history(
