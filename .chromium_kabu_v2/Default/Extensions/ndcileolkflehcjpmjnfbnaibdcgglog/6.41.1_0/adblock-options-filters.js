...
/* global activateTab, connectUIPort, delayedSubscriptionSelection,
   ...
   MABPayment, premiumFiltersCtaKey, selected, send,
...
const filtersAdblockUiServerLogger = new modulesAsGlobal.ewe.telemetry.ServerLogger("adblock_ui");
const filtersExtensionsUIServerLogger = new modulesAsGlobal.ewe.telemetry.ServerLogger(
  "extensions_ui",
);
...
FilterListUtil.sortFilterListArrays = (newFilters) => {
  ...
  const newFilterArray = newFilters
    .map((filterId) => otherSection.find(({ adblockId }) => adblockId === filterId))
    .filter(Boolean);
...
//   -- moves the lists called out for highlighting to the top of their section
// Inputs:
//    subs:object - Map for subscription lists taken from the background.
//    toHightlight:array - An array of filter list names to be moved to the top of their section.
...
function getDefaultFilterUI(filterList, checkboxID, filterListType) {
  ...
  const aaPrivacy = FilterListUtil.cachedSubscriptions.acceptable_ads_privacy;
  const testId = `filters_${filterList.adblockId}`;
...
function getToggleFilterUI(filterList, checkboxID) {
  const $checkBox = $("<input>")
    ...
    .attr("adblockId", checkboxID)
    .attr("id", checkboxID)
    .attr("data-testid", `filters_${filterList.adblockId}`)
...
CheckboxForFilterList.prototype = {
  ...
  bindActions() {
    ...
    this.checkBox.on("change", function filterListSelectionChanged(event) {
      ...
      // fire event after subscriptions have been validated.
      if (!event.addedViaBackground && isAcceptableAds(that.filterList)) {
        filtersExtensionsUIServerLogger.behavior("acceptable_ads_toggled", {
...
async function initPremiumFiltersCTA() {
  ...
  if (!userClosedpremiumFiltersCTA) {
    ...
    $("#btnPremiumFilterLearnMore").on("click", () => {
      filtersAdblockUiServerLogger.behavior("premium_filter_list_cta", { action: "clicked" });
      storageSet(premiumFiltersCtaKey, true);
      $("#premium-filter-list-cta").fadeOut(1000);
      activateTab("#mab");
    });
    filtersAdblockUiServerLogger.behavior("premium_filter_list_cta", { action: "seen" });
  }
}
