...
(function () {
  ...
  function add(target, filter, subscriptions) {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      oldRecord.filter = newRecord.filter;
      changes.push({
  ...
  function getFilterInfo(filter, subscriptions) {
    ...
    return {
      ...
      csp: filter.csp,
      isAddHeader: filter.isAddHeader,
  ...
  function hasRecord(newRecord, oldRecord) {
    ...
    if (
      oldRecord.filter &&
      (oldRecord.filter.csp || oldRecord.filter.isAddHeader) &&
      newRecord.filter &&
      (newRecord.filter.csp || newRecord.filter.isAddHeader)
    ) {
