...
chrome.runtime.onMessage.addListener((msg, sender, sendRes) => {
    if (sender.id != chrome.runtime.id || !msg?.op) {
        return;
    }
    switch (msg.op) {
        case "defaultPalette":
            return void sendRes(bg.defaultPalette);
        case "color_sources":
            return void sendRes(bg.color_sources);
        default:
            msg.params ??= [];
            return void sendRes(bg[msg.op](...msg.params));
    }
});
async function injectScriptsAsync(tabId, scripts, allFrames = false) {
...
async function injectEDropperAsync(tabId, allFrames) {
    await injectScriptsAsync(tabId, [
        "/js/helpers.js",
        "/js/vendor/shortcut.js",
        "/js/vendor/scrollStop.js",
        "/js/overlay.js",
        "/js/rect.js",
        "/js/edropper2.js",
    ], allFrames);
...
const bg = {
    ...
    injectDropper: async function () {
        console.log('bg: injecting dropper scripts');
        await injectEDropperAsync(bg.tab.id, false);
        console.log('bg: edropper2 injected');
        bg.pickupActivate();
    },
    refreshDropper: async function () {
        console.log('bg: refreshing dropper scripts');
        await injectEDropperAsync(bg.tab.id, true);
    ...
    setColor: function (color, history = true, source = 1, url) {
    ...
    saveToHistory: function (color, source = 1, url) {
        var palette = bg.getPalette();
        if (!palette.colors.find((x) => {
            return x.h == color;
        })) {
    ...
    tabOnChangeListener: function () {
        ...
        chrome.tabs.onActivated.addListener((info) => {
            if (bg.tab && bg.tab.id == info.tabId)
                bg.sendMessage({
    ...
    isPalette: function (name) {
        return bg.history.palettes.find((x) => {
            return x.name == name;
        })
            ? true
            : false;
    ...
    getPaletteNames: function () {
        return bg.history.palettes.map((x) => {
            return x.name;
        });
    },
    uniquePaletteName: function (name) {
        ...
        else if (bg.getPaletteNames().find((x) => {
            return x == name;
        })) {
     ...
     * f = favorite
     */
    historyColorItem: function (color, timestamp = Date.now(), source = 1, favorite = false, _url) {
        return {
            h: color,
            n: '',
            s: source,
