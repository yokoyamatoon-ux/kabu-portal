...
const adShownEvent = "yt-site.ad-shown";
const toastShownEvent = "yt-site.toast-shown";
const contextTamperingEvent = "yt-site.context-tampering";
const errorEvent = "yt-site.yt-error";
var ErrorType;
(function (ErrorType) {
...
const ytErrorLoggingFlag = "yt-uncaught-exception-logging";
...
const ytHavenAutoAllowlistVideo = "yt-haven-allowlist-video";
...
;// CONCATENATED MODULE: ../../fragment/yt-wall-detection/dist/shared/index.js
...
function handleMutations(mutations) {
    for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
            ...
            const elements = [
                node,
                ...Array.from(node.querySelectorAll([adWallSelector, toastSelector, ...playerErrorSelectors].join(","))),
            ];
...
;// CONCATENATED MODULE: ../../fragment/yt-wall-detection/dist/img/check.js
...
;// CONCATENATED MODULE: ../../fragment/yt-wall-detection/dist/img/cross.js
const crossIcon = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none">
...
const SURVEY_URL = "https://survey.alchemer.eu/s3/90847063/Quick-Feedback";
...
function createToggleSwitch(initialState = false, onToggle = async () => { }) {
    ...
    const colors = {
        ENABLED: "#3c5b81",
        DISABLED: isDarkMode() ? "rgba(255, 255, 255, 0.15)" : "#b0c0d1",
    };
    const toggle = document.createElement("div");
    Object.assign(toggle.style, {
        width: "36px",
        height: "18px",
        backgroundColor: initialState ? colors.ENABLED : colors.DISABLED,
        borderRadius: "18px",
        position: "relative",
        cursor: "pointer",
    ...
    thumb.innerHTML = isActive ? checkIcon : crossIcon;
    toggle.appendChild(thumb);
    toggle.addEventListener("click", () => {
        const newState = !isActive;
        if (newState !== isActive) {
            isActive = newState;
            toggle.style.backgroundColor = isActive
                ? colors.ENABLED
                : colors.DISABLED;
...
function createMenuComponent() {
    ...
    const createItem = (emoji, i18nKey, href = "") => {
    ...
    menu.appendChild(createItem("🥲", "yt_haven_feedback_fix_it", SURVEY_URL));
    menu.appendChild(createItem("👎", "yt_haven_feedback_hate", SURVEY_URL));
...
function addInfoBox(targetContainer, toggleState, onToggle) {
    ...
    Object.assign(infoBoxElem.style, {
        marginTop: "12px",
        backgroundColor: isDarkMode() ? "#272727" : "#dff1fe",
        borderRadius: "12px",
        padding: "12px",
        fontSize: "14px",
        color: isDarkMode() ? "#fff" : "#000",
