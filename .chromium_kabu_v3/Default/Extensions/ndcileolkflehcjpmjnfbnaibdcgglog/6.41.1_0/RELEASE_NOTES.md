...
- Updated snippets to 2.9.0
- Fixed: Marketing messages opt-out had no effect on on-page dialogs.
...
- Added extension state to the Support tab, showing active filter lists, filter
  and rule counts, and DNR dynamic rule usage.
...
- Fixed upsell banner text not visible on dark theme in options page.
- Source code release bundles no longer include the full monorepo, but instead
  only include the code to build the current extension.
- Updated the reverse trial messaging to be less obtrusive and make it clear
  that it is free and reverts to standard blocking automatically at the end of
  the trial.
...
- Excluded all parameters from install URL and uninstall URL when the user is opted out of
  data collection.
- Add a reverse trial campaign to give some users a free preview of our improved
  premium snippet offering.
...
- Fixed additional parameters being set on the uninstall URL when the users is
...
- Updated snippets to 2.6.0

# 6.36.0 - 2026-02-17

- Restructured UserProfile API schema to product-specific format. Trial information now in `subscription` object, preferences in `preferences` object. API methods simplified to return product-specific data. (noissue)

# 6.35.0 - 2026-02-10

- Updated account API to use products nested structure with trial status, trial days left, and consent preferences per product (ab/abp). (EMP-42)
- Added getTrial(), getConsents(), and getTrialDaysLeft() methods to core SDK account module. (EMP-42)
- Added isTrial field to extension metadata injection on product websites. (EMP-42)

# 6.34.0 - 2026-02-04

- Snippets are now initialized at the SDK level.

# 6.33.6 - 2026-01-27

This release contains only minor updates and under-the-hood changes.

- Updated snippets to 2.5.0

# 6.33.5 - 2026-01-20

This release contains bugfixes and under-the-hood changes.

- Fixed: Update campaign for Firefox users should only be triggered every other
  week (EXT-1121).

# 6.33.4 - 2026-01-13

- In Firefox, fixed a bug where regex filters that don't use the `match-case`
  option were lowercased, which broke capital letter metacharacters. This bug
  sadly still affects Chromium-based MV3 browsers. (#12)
- Fixed `$genericblock` exception filters incorrectly allowing `$addheader`
  rules in MV2 and not being logged in MV2/MV3 (EXT-1126).
- Fixed UI pages that broke in Chrome 144.
...
- Add support for $addheader filter type (EXT-1098).
...
- Removed "Allow AdBlock to show me messages on webpages when relevant" option
  in favor of option to opt-out of all marketing messages (EXT-1080).

# 6.31.1 - 2025-11-04

This release contains bugfixes and under-the-hood changes.

- Removed experiment setup for tracker blocking as premium feature (EXT-1009)
...
This release removes Zendesk integration (EXT-1089).

# 6.29.0 - 2025-09-23

- Added user account integration. (EXT-1013)
- On extension update, DNR dynamic rules will now be checked against our
  expected filters. This was previously tested behind an experiment flag, but is
  now enabled for everybody. (EXT-938)

# 6.28.0 - 2025-09-16

This release fixes an occasional bug and removes some old features that
are not used anymore.

- Fixed a bug that sometimes caused an error message to be shown when clicking
  on the "Support" button in the icon popup
- Removed Sync Settings feature. (EXT-1076)
- Removed allowlist YouTube channel feature (EXT-1079)
- Removed allowlist Twitch channel feature (EXT-1081)
...
- Updated allowlisted icon to grey (noissue)
- Updated snippets to 2.4.1

# 6.27.1 - 2025-08-27

This release contains bugfixes and under-the-hood changes.

- Fixed DNR dynamic rules getting out of sync with our dynamic filters in MV3
  extensions. (EXT-938)
- Updated snippets to 2.4.0

# 6.27.0 - 2025-08-18

This release removes the keyboard shortcut for pausing AdBlock globally.

- Remove shortcut for global pause
- Fixed missing user ID (EXT-1001)

# 6.26.0 - 2025-08-12

- Changed the extension icon from green to grayscale when the tab is allowlisted (EXT-1049).
- Added auto-allowlist per video (EXT-1050)

# 6.25.2 - 2025-08-04

This release contains bugfixes and under-the-hood changes.

- Fixed initialization of experiments, cohorts, and logger after the service
  worker restarts (noissue)
- Fixed uninstall URL after experiments and cohorts were changed (noissue)
...
- Fixed an issue with Pause on all sites feature (EXT-911)
- Removed encoded license from Bypass and Public APIs
- Technical debug data is no longer added to bug reports on Firefox

# 6.24.1 - 2025-07-15

This release contains filter updates and under-the-hood changes.

# 6.24.0 - 2025-07-08

- Add website-specific 'cookie consent' CTA. (GROW-56)
...
- Fixed a display issue with the filter hits devtools. (EE-894)
